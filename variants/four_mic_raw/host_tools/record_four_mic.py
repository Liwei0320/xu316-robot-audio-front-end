    ...
    while True:
        ...
        for index, device in enumerate(devices):
            ...
            if (
                ...
                and ("XU316" in name or "Beamformer" in name)
            ):
                return index, name

        if time.monotonic() >= deadline:
            raise RuntimeError(
                "XU316 input was not found after 30 seconds. "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Record four channels from XU316.")
    parser.add_argument(
        "-t", "--seconds", type=float, default=10.0,
        help="Recording duration in seconds (default: 10).",
    )
    parser.add_argument(
        "-o", "--output", default="xu316_recording.wav",
...
def main():
    ...
    print("Channels: DATA0, DATA1, DATA2, DATA3")
