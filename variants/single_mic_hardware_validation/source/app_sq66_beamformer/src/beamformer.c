...
#if ((BF_DELAY_LINE_LENGTH & (BF_DELAY_LINE_LENGTH - 1)) != 0)
#error BF_DELAY_LINE_LENGTH must be a power of two
#endif
                                      ...
                                      int32_t input)
{
    ...
    output += ((int64_t) BF_DC_BLOCK_ALPHA_Q31
    ...
    output = ((int64_t) filtered * state->gain_q15[microphone]) >> 15;
                                     ...
                                     unsigned microphone)
{
    ...
    return saturate_i32((int64_t) newer
        + ((delta * fraction) >> BF_DELAY_FRACTION_BITS));
}
...
void beamformer_init(beamformer_state_t *state)
{
    ...
    static const uint16_t delays[BF_MIC_COUNT] = {
        BF_DELAY_Q8_0,
        BF_DELAY_Q8_1,
        BF_DELAY_Q8_2,
        BF_DELAY_Q8_3
...
void beamformer_set_delays_q8(beamformer_state_t *state,
                              const uint16_t delays_q8[BF_MIC_COUNT])
{
    for (unsigned microphone = 0; microphone < BF_MIC_COUNT; ++microphone) {
        uint16_t delay = delays_q8[microphone];
        if (delay > BF_MAX_DELAY_Q8) {
            delay = BF_MAX_DELAY_Q8;
        }
        state->delay_q8[microphone] = delay;
    }
}
                           ...
                           const int32_t microphones[BF_MIC_COUNT])
{
    ...
    return post_process_sample(&state->post,
                               saturate_i32(sum / BF_MIC_COUNT));
}
